...
public class BoardController {
    ...
    @GetMapping("/board/{id}")
    public String detail(@PathVariable Integer id ,HttpServletRequest request) {  //C (모델의 데이터를 가져와서)
        User sessionUser = (User) session.getAttribute("sessionUser");
        Board board = boardRepository.finById(id); //M
      ...
      request.setAttribute("pageOwner", true);
        return "board/detail";  //v (뷰를 )
    }
}